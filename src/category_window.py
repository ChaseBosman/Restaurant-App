...
class CategoryWindow(Toplevel):
    ...
    def populate_category_buttons(self):
        ...
        with FoodDbOperations() as cur:
            ...
            for row in drinks:
                print(row[0])
                self.category_buttons.append(Button(self, text=row[0]))
                self.category_buttons[self.category_count_drinks].grid(row=self.category_count_drinks, column=0)
            ...
            for row in food:
                ...
                self.category_buttons[self.category_count_food+self.category_count_drinks].\
                    grid(row=self.category_count_food, column=15)
